...
def callback(msg):
    ...
    if send_to_gripper(z):
        return_to_saved_position(pub, x, y)
    else:
        rospy.logwarn("Skipping return because grip confirmation failed.")
 
def send_to_gripper(z):
    import socket
    HOST = "10.0.1.27"  # Or IP address
    PORT = 5005
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            s.sendall(f"0.0,0.0,{z}\n".encode())
            rospy.loginfo(f"[MOVEMENT] Sent z={z:.2f} to gripper")
        ...
        rospy.logwarn(f"[MOVEMENT] Failed to send z to Pi: {e}")
